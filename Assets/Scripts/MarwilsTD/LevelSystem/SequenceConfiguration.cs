...
{
    [Serializable]
    public class SequenceConfiguration : Runner, IRouteProvider
    {
        [SerializeField]
        [Tooltip("The routes that define the spawn points and target points for the wave sequence")]
        private RouteConfiguration _route;
        public RouteConfiguration Route => _route;

        [SerializeField]
        private List<SequenceElementConfiguration> _sequenceElements = new();
        public IReadOnlyList<SequenceElementConfiguration> SequenceElements => _sequenceElements;
