        ...
        public void DrawSpawnPlanContent(Rect position, SpawnPlanConfiguration spawnPlan, ref float currentY)
        {
            ...
            if (sequencesProp != null)
            {
                for (int i = 0; i < sequencesProp.arraySize; i++)
                {
                    ...
                    var sequenceHeight = sequenceProp.isExpanded
                        ? _sequenceDrawer.GetPropertyHeight(sequenceProp, sequenceGuiLabel)
                        : 0;
                    var totalHeight =
                        sequenceHeight
                    ...
                    var foldoutRect = new Rect(
                        position.x + Constants.MarginVertical,
                        currentY + Constants.MarginVertical,
                        15,
                        EditorGUIUtility.singleLineHeight
                    );
                    sequenceProp.isExpanded = EditorGUI.Foldout(
                        foldoutRect,
                        sequenceProp.isExpanded,
                        GUIContent.none,
                        true
                    );
                    ...
                    var headerWaveRect = new Rect(
                        position.x + Constants.MarginVertical,
                        currentY + Constants.MarginVertical,
                        position.width - 60 - Constants.MarginVertical,
                        EditorGUIUtility.singleLineHeight
                    );
                    var headerStyle = new GUIStyle(EditorStyles.boldLabel);
                    headerStyle.fontSize = 14;
                    ...
                    if (GUI.Button(deleteMinusButtonRect, "-"))
                    {
                        if (sequence != null)
                        {
                            ...
                            bool isFactorySequence =
                                routeName.StartsWith("SpawnPlanSequence_") && routeName.Length >= 35;
        ...
        public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
        {
            ...
            if (sequencesProp != null)
            {
                for (int i = 0; i < sequencesProp.arraySize; i++)
                {
                    var sequenceProp = sequencesProp.GetArrayElementAtIndex(i);
                    var sequenceHeight = sequenceProp.isExpanded
                        ? _sequenceDrawer.GetPropertyHeight(sequenceProp, new GUIContent($"Sequence {i + 1}"))
                        : 0;
